...
요런 방식은 너무 비효율적이고, 앞으로도 이런 경우가 자주 생길 것 같아서 직접 보일러플레이트 생성기를 만들어보기로 하였다!!
...
먼저 package.json의 `bin` 설정을 해서, 패키지를 설치할 때, 실행될 스크립트 경로를 넣어주었다.
...
이 때, `run` 스크립트는 어떠한 사용자도 모두 실행할 수 있도록 권한을 변경해준다.
...
- 먼저 (좋은 문법인지는 모르겠지만,,) options에 키값이 없을 경우에는 default로 적용하고 키값이 있으면 해당 키값을 사용하도록 설정해주었다.
