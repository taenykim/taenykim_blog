...
이는 함수를 최대한 자기 기능만 수행할 수 있도록 쪼개는 것을 요구한다. 함수를 쪼개므로서, 가독성과 유지보수가 좋아지며, 자기 기능만 수행하는 함수를 최대한 쪼개므로써, 안정성은 올라간다.
...
자바스크립트는 다른 객체지향 프로그래밍 언어들에 비해 더 유연한 특징을 가지지만 객체지향 프로그래밍을 지원하며, 또한 함수형 프로그래밍도 지원한다. 완벽하게 객체지향적으로 코드를 작성하고 모든 함수를 함수형 프로그래밍으로 작성하는 것은 어렵겠지만 지향점을 잘 파악하고 최대한 **좋은 코드**를 작성하고자 하는 것이 중요하지 않을까? 생각해보았다.

<hr/>

## 6. 반복문 끊어주기 (break, return)

사용자가 입력한 자동차이름이 5글자 초과인지 검사하는 [함수](https://github.com/taenykim/java-racingCar)를 기존에는 입력한 값의 길이만큼 반복문을 돌면서 조건을 만족하면 errorCheck 변수를 바꾸도록 했었다.
...
해당 코드는 중간에 조건이 만족하여도 불필요하게 모든 반복문을 수행한다. 하지만 조건을 만족했을 때 바로 값을 return 하도록 바꿨더니 변수를 쓰지않아도 되고, 조건 만족시, 반복문도 전부 탐색하지 않아도 되었다.
